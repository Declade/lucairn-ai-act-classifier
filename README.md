...
> **Cache file contents:** each cache entry stores the full `ExtractedFeatures` object including the original input text (so the rules engine can re-render its citation chain on a hit). Cache files are written with mode 0600 (user-only on POSIX systems); the cache directory is created with mode 0700. The cache lives in your user-private `~/.cache/` directory and is not transmitted anywhere. For sensitive use-case descriptions, either run with `--no-cache` (one-off) or include `~/.cache/lucairn-ai-act-classifier/` in your session-cleanup routine (recurring).
