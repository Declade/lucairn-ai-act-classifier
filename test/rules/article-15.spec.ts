...
describe('classifyArticle15() — pure-function determinism', () => {
  it('returns the same output for the same input', () => {
    const annex = makeAnnex({ high_risk: true, domains: [4], suppressed_by_article_5: false });
    const art5 = makeArt5(false);
    const a = classifyArticle15(annex, art5);
    const b = classifyArticle15(annex, art5);
    expect(a).toEqual(b);
  });

  it('throws TypeError on non-object annex', () => {
    // @ts-expect-error: deliberately invalid
    expect(() => classifyArticle15(null, makeArt5(false))).toThrow(TypeError);
  });
  ...
  it('throws TypeError on annex shapes missing the domains array (e.g. {}, [])', () => {
    ...
    // @ts-expect-error: deliberately invalid
    expect(() => classifyArticle15({}, makeArt5(false))).toThrow(TypeError);
    // @ts-expect-error: deliberately invalid
    expect(() => classifyArticle15([], makeArt5(false))).toThrow(TypeError);
  });
  ...
  it('throws TypeError on non-object article5', () => {
    // @ts-expect-error: deliberately invalid
    expect(() => classifyArticle15(makeAnnex({ high_risk: false, domains: [], suppressed_by_article_5: false }), null)).toThrow(TypeError);
  });
});

describe('classifyArticle15() — applicable: true cascade (multiple Annex III domains)', () => {
  it('biometric high-risk (Annex III.1) → applicable === true', () => {
    ...
    const result = classifyArticle15(annex, makeArt5(false));
  ...
  it('multiple Annex III domains → triggered_by lists them sorted ascending', () => {
    const annex = makeAnnex({ high_risk: true, domains: [5, 1, 8], suppressed_by_article_5: false });
    const result = classifyArticle15(annex, makeArt5(false));
    expect(result.applicable).toBe(true);
    expect(result.triggered_by.annex_iii_domains).toEqual([1, 5, 8]);
  });
});

describe('classifyArticle15() — applicable: false when not high-risk', () => {
...
describe('classifyArticle15() — applicable: false when suppressed by Article 5', () => {
  it('suppressed_by_article_5 === true → applicable === false; triggered_by.article_5 === true', () => {
    ...
    const result = classifyArticle15(annex, makeArt5(true));
    expect(result.applicable).toBe(false);
...
describe('classifyArticle15() — summary spot-check + source URL', () => {
  it('summary_en contains the verbatim EUR-Lex phrase "accuracy, robustness, and cybersecurity"', () => {
    const annex = makeAnnex({ high_risk: true, domains: [4], suppressed_by_article_5: false });
    const result = classifyArticle15(annex, makeArt5(false));
    expect(result.summary_en).toContain('accuracy, robustness, and cybersecurity');
  });

  it('summary_de contains the verbatim EUR-Lex phrase "Genauigkeit, Robustheit und Cybersicherheit"', () => {
    const annex = makeAnnex({ high_risk: true, domains: [4], suppressed_by_article_5: false });
    const result = classifyArticle15(annex, makeArt5(false));
    expect(result.summary_de).toContain('Genauigkeit, Robustheit und Cybersicherheit');
  });
