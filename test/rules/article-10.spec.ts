...
// Synthetic AnnexIIIResult builders — these tests exercise classifyArticle10
// in isolation. We do NOT depend on extractFeatures + classifyAnnexIII to
// construct inputs because the cascading rule modules should be testable
// independently of the upstream pipeline (Day-3 lesson: pure-function consumers
...
describe('classifyArticle10() — pure-function determinism', () => {
  it('returns the same output for the same input', () => {
    const annex = makeAnnex({ high_risk: true, domains: [4], suppressed_by_article_5: false });
    const art5 = makeArt5(false);
    const a = classifyArticle10(annex, art5);
    const b = classifyArticle10(annex, art5);
    expect(a).toEqual(b);
  });
...
describe('classifyArticle10() — applicable: true cascade (multiple Annex III domains)', () => {
  it('biometric high-risk (Annex III.1) → applicable === true', () => {
    ...
    const result = classifyArticle10(annex, makeArt5(false));
  ...
  it('multiple Annex III domains → triggered_by lists them sorted ascending', () => {
    // Note: we feed domains out-of-order to confirm classifyArticle10 sorts defensively.
    const annex = makeAnnex({ high_risk: true, domains: [8, 1, 4], suppressed_by_article_5: false });
    const result = classifyArticle10(annex, makeArt5(false));
    expect(result.applicable).toBe(true);
    expect(result.triggered_by.annex_iii_domains).toEqual([1, 4, 8]);
  });
});

describe('classifyArticle10() — applicable: false when not high-risk', () => {
...
describe('classifyArticle10() — applicable: false when suppressed by Article 5', () => {
...
describe('classifyArticle10() — summary spot-check + source URL', () => {
  it('summary_en contains the verbatim EUR-Lex phrase "training, validation and testing data sets"', () => {
    const annex = makeAnnex({ high_risk: true, domains: [4], suppressed_by_article_5: false });
    const result = classifyArticle10(annex, makeArt5(false));
    expect(result.summary_en).toContain('training, validation and testing data sets');
  });

  it('summary_de contains the verbatim EUR-Lex phrase "Trainings-, Validierungs- und Testdatensätzen"', () => {
    const annex = makeAnnex({ high_risk: true, domains: [4], suppressed_by_article_5: false });
    const result = classifyArticle10(annex, makeArt5(false));
    expect(result.summary_de).toContain('Trainings-, Validierungs- und Testdatensätzen');
  });
