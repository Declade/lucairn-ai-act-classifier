...
describe('classifyArticle12() — pure-function determinism', () => {
  it('returns the same output for the same input', () => {
    const annex = makeAnnex({ high_risk: true, domains: [4], suppressed_by_article_5: false });
    const art5 = makeArt5(false);
    const a = classifyArticle12(annex, art5);
    const b = classifyArticle12(annex, art5);
    expect(a).toEqual(b);
  });

  it('throws TypeError on non-object annex', () => {
    // @ts-expect-error: deliberately invalid
    expect(() => classifyArticle12(null, makeArt5(false))).toThrow(TypeError);
  });
  ...
  it('throws TypeError on annex shapes missing the domains array (e.g. {}, [])', () => {
    ...
    // @ts-expect-error: deliberately invalid
    expect(() => classifyArticle12({}, makeArt5(false))).toThrow(TypeError);
    // @ts-expect-error: deliberately invalid
    expect(() => classifyArticle12([], makeArt5(false))).toThrow(TypeError);
  });
...
describe('classifyArticle12() — applicable: true cascade (multiple Annex III domains)', () => {
  it('biometric high-risk (Annex III.1) → applicable === true', () => {
    ...
    const result = classifyArticle12(annex, makeArt5(false));
  ...
  it('multiple Annex III domains → triggered_by lists them sorted ascending', () => {
    // ≥3 out-of-order elements (Day-4 Codex C32 lesson: 2-element sort is just
    // a swap; only ≥3 elements exercise actual sort logic).
...
describe('classifyArticle12() — applicable: false when not high-risk', () => {
...
describe('classifyArticle12() — applicable: false when suppressed by Article 5', () => {
  it('suppressed_by_article_5 === true → applicable === false; triggered_by.article_5 === true', () => {
    ...
    const result = classifyArticle12(annex, makeArt5(true));
    expect(result.applicable).toBe(false);
  ...
  it('inconsistent upstream state (high_risk === true AND suppressed_by_article_5 === true) → suppression wins', () => {
    // Defensive invariant lock (Day-4 M-1 closure pattern). In normal upstream
    // flow, classifyAnnexIII() sets high_risk: false whenever suppression
    // fires, so this cell should never occur in practice. We test it anyway
    // to pin the cascade behavior in case the upstream invariant ever drifts:
    // suppression is the operative signal regardless of high_risk.
...
describe('classifyArticle12() — summary spot-check + source URL', () => {
  it('summary_en contains the verbatim EUR-Lex phrase "automatic recording of events (logs)"', () => {
    const annex = makeAnnex({ high_risk: true, domains: [4], suppressed_by_article_5: false });
    const result = classifyArticle12(annex, makeArt5(false));
    expect(result.summary_en).toContain('automatic recording of events (logs)');
  });

  it('summary_de contains the verbatim EUR-Lex phrase "automatische Aufzeichnung von Ereignissen"', () => {
    const annex = makeAnnex({ high_risk: true, domains: [4], suppressed_by_article_5: false });
    const result = classifyArticle12(annex, makeArt5(false));
    expect(result.summary_de).toContain('automatische Aufzeichnung von Ereignissen');
  });

  it('summary_de contains the EUR-Lex Lebenszyklus phrasing (verbatim Tier-3 chapeau)', () => {
    const annex = makeAnnex({ high_risk: true, domains: [4], suppressed_by_article_5: false });
    const result = classifyArticle12(annex, makeArt5(false));
    expect(result.summary_de).toContain('während des Lebenszyklus des Systems');
  });
